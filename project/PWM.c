...
void PWM_Init(void) {
	...
	// set the prescaler value 3
	// this is because input f = 4MHz
	// and the scaled frequency needs to be 1MHz
	// so this needs to be 4M/(3+1) = 1M
	TIM1->PSC &= ~TIM_PSC_PSC;
	TIM1->PSC |= 0X03;
	
	// set the ARR value 512
	// in upcounting mode so
	// counting period = (1+ARR)x(1/freq(clock))
